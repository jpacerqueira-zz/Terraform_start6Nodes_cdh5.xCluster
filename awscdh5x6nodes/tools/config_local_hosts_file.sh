...
updateHostsFile() {
  ...
  echo ""
  invalidIP=0
  echo "What is the EC2 public IP address of your first machine (dn1 elephant)?"
  read ip1
  if [[ $ip1 == 10.* || $ip1 == 172.* || $ip1 == 192.168.* ]]; then
    invalidIP=1
  fi
  echo "What is the EC2 public IP address of your second machine (dn2 tiger)?"
  read ip2
  if [[ $ip2 == 10.* || $ip2 == 172.* || $ip2 == 192.168.* ]]; then
    invalidIP=1
  fi
  echo "What is the EC2 public IP address of your third machine (nn1 horse)?"
  read ip3
  if [[ $ip3 == 10.* || $ip3 == 172.* || $ip3 == 192.168.* ]]; then
    invalidIP=1
  fi
  echo "What is the EC2 public IP address of your fourth machine (nn2 monkey)?"
  read ip4
  if [[ $ip4 == 10.* || $ip4 == 172.* || $ip4 == 192.168.* ]]; then
    invalidIP=1
  fi
  echo "What is the EC2 public IP address of your fourth machine (nn3 lion)?"
  read ip5
  if [[ $ip5 == 10.* || $ip5 == 172.* || $ip5 == 192.168.* ]]; then
    invalidIP=1
  fi
  fi
  echo "What is the EC2 public IP address of your fourth machine (nn4 snake)?"
  read ip6
  if [[ $ip6 == 10.* || $ip6 == 172.* || $ip6 == 192.168.* ]]; then
    invalidIP=1
  fi
  ...
  sudo sh -c "echo $ip5 dn3lion >> /etc/hosts"
